import * as vars from './js/variables.mjs'

const random_gallery = document.getElementById('kittens_gallery');
const favourites_gallery = document.getElementById('favourites__kittens');
const reloadBtn = document.getElementById('callAnotherCat');

export const callRandomCat = async () => {
    let data = await (await fetch(vars.API_RANDOM)).json();
...
export const callFavCats = async () => {
    ...
    let data = await (await fetch(vars.API_FAVOURITES, {
        method: 'GET',
        headers: {
...
export const deleteCatInFavourites = async (cat_id) => {
    const res = await fetch(`${vars.API_FAVOURITES_DELETE(cat_id)}`, {
        method: 'DELETE',
        headers: {
            'x-api-key': vars.API_KEY
        }
...
callRandomCat();
callFavCats();
reloadBtn.addEventListener('click', callRandomCat);
